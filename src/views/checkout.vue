					...
					<div class="box-inner js-checkout-address-panel ">
						<div class="address-common-table js-multiple-address-panel">
							<ul class="address-item-list clear js-address-item-list">
								<li class="js-choose-address" :class="{'selected-address-item' :receiveIndex==index}" v-for="receive,index in receiveInfo"
								 @click="chooseReceive(index)">
									<div class="address-item">
										<div class="name-section"> {{receive.name}} </div>
										<div class="mobile-section">{{receive.phone}}</div>
										<div class="detail-section">{{receive.province}} {{receive.city}} {{receive.county}} <br> {{receive.add}}
										</div>
									</div>
					...
					<div class="invoice-detail"> 发票抬头：
						<div class="radio-box">
							<label>
								...
								<span class="blue-radio" :class="{'blue-radio-on':invoice.personal}" @click="checkedInvoice(true)"><a></a></span>
								个人
							</label>
							<label>
								<input type="radio" class="hide">
								<span class="blue-radio" :class="{'blue-radio-on' :!invoice.personal}" @click="checkedInvoice(false)"><a></a></span>
								单位
							</label>
						</div>
			...
			<div class="gray-box">
						...
						<div class="cart-group js-cart-group">
							<div class="cart-items" v-for="item ,index in checkedGoods">
								...
								<div class="name hide-row">
									<div class="name-cell">
										<a href="http://www.smartisan.com/shop/#/item/100027501" :title="item.title+'（'+item.spec_json.show_name+'）'"
										 target="_blank">{{item.title}}（{{item.spec_json.show_name}}）</a>
									</div>
								</div>
								<div class="subtotal">
				...
				<div class="box-inner">
					<div class="order-discount-line">
						<p> 商品总计： <span>¥ {{totlePrice}}.00</span> </p>
				...
				<div class="box-inner">
					<div class="last-payment clear">
						<span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrderHandle">
							<a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{totlePrice+freight}}.00</em></span>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		created() { //在实例创建之后同步调用。此时实例已经结束解析选项，这意味着已建立：数据绑定，计算属性，方法，watcher/事件回调
		...
		methods: { //methods必须要有一定的触发条件才能执行
			chooseReceive(index) {
				this.receiveIndex = index
			},
			closePop() {
