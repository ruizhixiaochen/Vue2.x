	...
	export default {
		...
		methods: {
			...
			editReceive(data, index) {
				...
				this.popShow = true
				this.$store.state.editCheck = true
				this.$store.state.editIndex = index
			},
			deleteReceive(index) {//删除地址
				this.$axios.post('/api/user/addressDelete', {
							'id': this.$store.getters.currentUser,
							'name': this.$store.state.receiveInfo[index].name,
							'phone': this.$store.state.receiveInfo[index].phone,
							'areaCode': this.$store.state.receiveInfo[index].areaCode,
							'landLine': this.$store.state.receiveInfo[index].landLine,
							'county': this.$store.state.receiveInfo[index].county,
							'add': this.$store.state.receiveInfo[index].add,
							'df': this.$store.state.receiveInfo[index].default
						})
						.then(res => {
						})
						.catch(error => {
				 ...
				 this.$store.state.receiveInfo.splice(index, 1)
			}
		}
	}
</script>
