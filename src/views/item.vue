			...
			<div class="item-box">
				<div class="gallery-wrapper">
					<div class="gallery">
						<div class="thumbnail">
							<ul>
								<!-- 在实际项目中想要取得循环中的index值，以通过事件获取：当你触发一个事件时，可以顺便拿下他的值。而且不仅是index值，元素本身也可以拿到 index是索引值-->
								<li :class="{'on':index == imgIndex}" @click="tableImg(index)" v-for="img,index in itemsInfo.ali_images"><img
									 :src="img+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
							</ul>
						</div>
						<div class="thumb">
							<ul>
								<li :class="{'on':index == imgIndex}" v-for="img,index in itemsInfo.ali_images"><img :src="img+'?x-oss-process=image/resize,w_450/quality,Q_90/format,webp'"></li>
							</ul>
						</div>
					</div>
				</div>
	...
	export default {
		...
		computed: {
			itemsInfo() {
				let itemsInfo = this.itemsData.filter((item) => {
					return Number(item.sku_id) == Number(this.$route.query.itemId)
				})[0]
				return itemsInfo
			}
		},
		methods: {
			...
			addCarPanelHandle() {
				...
				this.$store.commit('addCarPanelData', itemsData)
			},
			addCount() {
				this.count++
