...
<script>
	...
	export default {
		...
		mounted() {//el 被新创建的 vm.$el 替换，并挂载到实例上去之后调用该钩子。
			this.receive.phone = this.$store.state.currentUser
		},
		watch: {
			'receive.countyId'() {
				this.countyList.forEach((county) => { //两个return，函数嵌套，内层函数的返回值也不会作为外层函数的返回值返回出去
		...
		computed: {
			cityList() {
				let cityList = []
				this.addList.forEach((province) => { //两个return，函数嵌套，内层函数的返回值也不会作为外层函数的返回值返回出去
				...
				if (this.receive.provinceId === '0') { //注意这个''
			...
			countyList() {
				let countyList = []
				this.cityList.forEach((city) => { //cityList是数组！！！
		...
		methods: {
			closePop() { //触发当前实例上的事件。附加参数都会传给监听器回调。 
        ...
        if(this.receive.name&&this.receive.phone.length==11&&this.receive.province&&this.receive.city&&this.receive.county&&this.receive.add){
          ...
          this.isRight = false
			...
			submitReceiveHandle(){
				if(this.isRight) {
					this.$store.commit('submitReceiveHandle',this.receive)
					this.$emit('close')
